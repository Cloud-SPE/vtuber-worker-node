...
- An Ethereum keystore for ticket redemption (per `livepeer-payment-library` receiver mode).
- An HTTPS-served `/.well-known/livepeer-registry.json` path (per `livepeer-service-registry` publisher mode).
...
- **Not chain-aware.** All wei-side logic lives in the `livepeer-payment-library` receiver daemon. This repo speaks gRPC to it over a unix socket and never touches ETH directly.
...
- **Wire compatibility with `livepeer-payment-library`.** This repo pins a tag; payments are byte-compatible with `net.Payment`. Drift is a release-blocking bug.
